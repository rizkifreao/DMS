...
class Trips extends CI_Controller
{
      ...
      if ($response) {
        if (!empty($this->input->post('e_expense_type'))) {
          foreach ($this->input->post('e_expense_type') as $key => $e_expense_type) {
            ...
            echo json_encode($arrangeexp);
          }
          $inserttripexp = $this->trips_model->add_trip_expense($arrangeexp);
        }
        $this->session->set_flashdata('successmessage', 'Data berhasil disimpan..');
      } else {
        $this->session->set_flashdata('warningmessage', 'Gagal menyimpan data.. Harap Cobalagi');
      }
      redirect('trips');
    } else {
  ...
  public function viewDetail($id)
  {
    ...
    if ($trip) {
      $data = [
        'trip' => $trip, 
        'det_trip_start' => $this->workorder_model->getDetailBy(['t_id' => $trip->t_id, 'p_pengeluaran' => 'start']),
        'det_trip_end' => $this->workorder_model->getDetailBy(['t_id' => $trip->t_id, 'p_pengeluaran' => 'end']),
        'vechicle'=>$this->vehicle_model->getDetail($trip->t_vechicle),
        'barang'=>$this->trips_model->getall_trips_expense($trip->t_id),
        'driver'=>$this->ion_auth->users($trip->t_driver)->row(),
      ];
      // echo json_encode($data);
  ...
  public function cetakSurat($id)
  {
    ...
    if ($trip) {
      ...
      // echo json_encode($data);
      $this->load->view('admin/trips/cetak',$data);
    }
  }
}
